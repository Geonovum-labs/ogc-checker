...
import { CC_3D_CURIE, CC_3D_URI } from '../rulesets/3d';
...
const is3dPlace = (input: unknown) =>
  ...
  TYPES_3D.includes(input.type);

const featureContains3dPlace = (input: unknown) =>
  input && typeof input === 'object' && 'place' in input && is3dPlace(input.place);
...
export const isValidConformanceTo3D: RulesetFunction<unknown> = async input => {
  if (featureContains3dPlace(input) || featureCollectionContains3dPlace(input)) {
    ...
    if (!conformsTo.includes(CC_3D_URI) && !conformsTo.includes(CC_3D_CURIE)) {
      return errorMessage(
